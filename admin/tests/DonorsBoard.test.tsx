...
import { act, fireEvent, render, screen as s, waitFor } from "@testing-library/react";
  ...
  it("checkbox selection works in DonorsBoard", async () => {
    await act(async () => {
      render(<DonorsBoard data={mockDonors} selectedIds={[1, 2, 3, 5]} />, {
        wrapper: ({children}) => <AdminContext dataProvider={testDataProvider({
          getMany: () => Promise.resolve({ data: mockDonors }),
      }) as DataProvider<'donors'>}><TestWrapper>{children}</TestWrapper></AdminContext>,
      });
    });
    ...
    await waitFor(()=> {
      ...
      expect(checkboxes.length).toBe(mockDonors.length);
    })
    const checkboxes = s.getAllByTestId("username");
    await act(async () => {
